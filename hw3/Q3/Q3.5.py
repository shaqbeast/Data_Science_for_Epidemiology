...
def simulate_strategy(name, G, k):
    n = G.number_of_nodes()
    i_frac = 4 / n
    beta = 0.005
    ...
    print("Peak Time - " + name + ": " + str(np.argmax(i_avg)))

    plt.plot(time, s_avg, label = "S")
    plt.plot(time, i_avg, label = "I")
    plt.xlabel("Time Step")
    plt.ylabel("Number of People")
    plt.title(name + " Strategy k=" + str(k))
    plt.legend()
    plt.show()
    ...
    print("Daily Infection Peak Time - " + name + ": " + str(np.argmax(daily_infections)))

    plt.plot(time, daily_infections)
    plt.xlabel('Time Steps')
    plt.ylabel('Number of People')
    plt.title('Daily Infected - ' + name + " Strategy k=" + str(k))
    plt.show()
...
simulate_strategy("Random", G_random, k)
...
simulate_strategy("Friends", G_friends, k)
...
simulate_strategy("Eigenvector Centrality", G_ev, k)