...
class CustomMLP(nn.Module):
    def __init__(self, input_size, hidden_size, output_size):
        ...
        self.hidden_layer = nn.Sequential(
            nn.Linear(input_size, 128),  # Increase to 128 or more
            nn.ReLU(),
            nn.Linear(128, 64),  # Add another hidden layer
            nn.ReLU()
        )
        self.output_layer = nn.Linear(hidden_size, output_size)  # No activation for regression
        self.bias = nn.Parameter(torch.Tensor([1.0]))

    def forward(self, x):
        x = x.view(x.size(0), -1)  # Flatten the sequence for each sample in the batch
        x = self.hidden_layer(x)
        x = self.output_layer(x) + self.bias
        return x
...
# Train and test data
task1 = weekly_counts[(weekly_counts['Year'] == 2020) & (weekly_counts['Epiweek'] != 0)]
task2 = weekly_counts[(weekly_counts['Year'] == 2021) & (weekly_counts['Epiweek'] != 0)]
...
benchmark = dataset_benchmark(train_datasets=[dataset1, dataset2], test_datasets=[dataset1, dataset2])
...
cl_strategy = SynapticIntelligence(
    ...
    train_epochs=100,    # Number of epochs per task
