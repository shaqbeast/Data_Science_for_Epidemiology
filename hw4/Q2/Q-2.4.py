...
cost_values_eps = np.zeros((simulations, max_time), dtype=float)
cost_values_soft = np.zeros((simulations, max_time), dtype=float)
...
for run in range(simulations):
    ...
    cost_values_eps[run] = cost_eps
    cost_values_soft[run] = cost_soft

avg_cost_eps = np.mean(cost_values_eps, axis=0)
avg_cost_soft = np.mean(cost_values_soft, axis=0)
time = []
for t in range(100):
    time.append(t)

plt.plot(time, avg_cost_eps, label='Epsilon Policy')
plt.plot(time, avg_cost_soft, label='Softmax Policy')
plt.xlabel('Time Steps')
plt.ylabel('Average Cost')
plt.legend()
plt.title('Multi-Arm Bandit Strategy for 2 Policies over 1000 runs')
plt.show()
