...
while s >= 1:
    ...
    if num_of_nodes_removed == 40:
        Gk_minus_2 = agg_Graph
    if num_of_nodes_removed == 41:
        Gk_minus_1 = agg_Graph
    if num_of_nodes_removed == 42:
        Gk = agg_Graph
...
t, S, I, R = EoN.fast_SIR(Gk_minus_2, beta, gamma)
plt.plot(t, I, label='Infected')
plt.xlabel('Time')
plt.ylabel('Fraction of Population')
plt.legend()
plt.title('SIR Model Gk-2 (β = ' + str(beta) + ' γ = ' + str(gamma) + ')')
plt.show()

t, S, I, R = EoN.fast_SIR(Gk_minus_1, beta, gamma)
plt.plot(t, I, label='Infected')
plt.xlabel('Time')
plt.ylabel('Fraction of Population')
plt.legend()
plt.title('SIR Model Gk-1 (β = ' + str(beta) + ' γ = ' + str(gamma) + ')')
plt.show()

t, S, I, R = EoN.fast_SIR(Gk, beta, gamma)
plt.plot(t, I, label='Infected')
plt.xlabel('Time')
plt.ylabel('Fraction of Population')
plt.legend()
plt.title('SIR Model Gk (β = ' + str(beta) + ' γ = ' + str(gamma) + ')')
plt.show()